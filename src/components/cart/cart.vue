<template>
  <Transition name="slide">
    <div
      v-if="props.open"
      v-click-away="closeCart"
      :class="{
        invisible: !props.open,
        'cart-container absolute z-40 flex shadow-md flex-col p-5 justify-between bg-white w-sm right-0 h-screen top-0': true,
      }"
    >
      <div>
        <div className="cart-heading">
          ...
          <img
            @click="closeCart"
            className="cart-close-img"
            src="@/components/icons/close-black.svg"
            width="35"
            fill="red"
            alt="close"
          />
        ...
        >
          ...
          >
            <img
              src="@/components/icons/empty.svg"
              width="80"
              alt="empty cart"
              loading="lazy"
            />
          ...
          <div
            v-else
            className="!mt-8 w-full justify-start overflow-auto items-start"
          >
            <CartItem
      ...
      <div>
        ...
        <button
          className="w-full text-white cursor-pointer px-4 py-2 bg-green-500 hover:bg-green-700"
        >
          Pay with stripe
        </button>
      </div>
  ...
  <Modal
    ...
    :icon="'error.svg'"
    :validate="validateRemoveProduct"
    :close-modal="closeModal"
    :message="modalContent"
    :action-message="'Are you sure to remove : '"
  ...
  if (cart && productToRemoveId.value)
    try {
      const data = {
        ...
        productId: productToRemoveId.value,
      };
      const response = await axios.delete(
        'http://localhost:5001/remove-product-from-cart',
        {
          data,
          headers: {
