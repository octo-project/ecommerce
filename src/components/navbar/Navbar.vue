<template>
  <nav
    className="sticky top-0 bg-gray-800 shadow-sm p-4 flex justify-between items-center"
  >
    <h1
      @click="goToHomePage"
      className="app-title text-white text-xl cursor-pointer"
    >
      E-Commerce
    </h1>
    <div className="navbar-menu flex items-center">
      <span v-if="store.connectedUser.pseudo" className="text-white !mr-5">
        Welcome
        <span className="!font-bold">{{ store.connectedUser.pseudo }} 👋</span>
      </span>
      <div className="flex items-center">
        <span
          id="toggle-cart-button"
          className="relative text-white cursor-pointer disabled-close !mr-5"
          @click="toogleCart"
        >
          <span
            v-if="productCount > 0"
            className="absolute disabled-close bg-blue-500 w-6 h-6 flex justify-center items-center text-center text-sm rounded-[50%] -top-[10px] -right-[15px]"
            >{{ productCount }}</span
          >
          <img
            width="25"
