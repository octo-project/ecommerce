<template>
  <div class="w-full min-h-screen">
    ...
    <div
      v-else
      className="product-detail-container gap-[20px] flex flex-col lg:flex-row p-5 items-center"
      :style="{ height: 'calc(100vh - 80px)' }"
    >
      <img
        :src="product.image"
        :alt="product.name"
        className="h-64 object-scale-down rounded"
        loading="lazy"
      />
      <div className="ml-0 !md:ml-5 pb-[15px] md:pb-0">
        <h1 className="text-2xl !font-semibold mt-4">{{ product.name }}</h1>
        <p className="product-description text-gray-600 !mt-2 text-justify">
          {{ product.description }}
        </p>
        ...
        <div>
          <button
            @click="goBack"
            className="mt-4 cursor-pointer bg-red-500 text-white px-4 py-2 rounded hover:bg-red-700 !mr-4"
          >
            Back
          </button>
          <button
            @click="addToCart"
            className="mt-4 cursor-pointer bg-green-500 text-white px-4 py-2 rounded hover:bg-green-700"
          >
            Add to Cart
          </button>
        </div>
...
onMounted(async () => {
  try {
    const response = await axios.get(
      `http://localhost:5001/product-detail/${route.params.id}`,
      {
        headers: {
...
const addToCart = async () => {
  ...
  try {
    const response = await axios.post(
      'http://localhost:5001/add-product-to-cart',
      cart,
      {
        headers: {
          Authorization: `Bearer ${authUser.token}`,
